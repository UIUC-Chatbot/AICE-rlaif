...
client = openai.OpenAI(
    ...
    api_key = "esecret_snv4qadkq2brnkg64jwty9uh5r"
)
...
## Load RAG paragraphs
data = pd.read_excel("./rag/gte_para.xlsx")
questions = data["question"].tolist()
paras = data["paragraph"].tolist()

## Clean paragraphs
for idx in range(3):
    paras[idx] = paras[idx].replace("\n", " ")
...
for idx in range(sample_num):
    prompts.append("\"" + paras[idx] + "\"" + "\n\nBased on this paragraph and true fact, " + questions[idx])
...
for idx in range(len(questions)):
    ...
    df_base.loc[df_baseRow, 'prompt'] = prompts[idx]
...
## save to excel
df_base.to_excel('./rag/result_org2.xlsx', sheet_name='Sheet1', index=False)